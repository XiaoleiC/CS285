...
\caption{MsPacman DQN: Training return (smoothed) and eval return plotted together.}
\label{fig:mspacman}
\end{figure}

\textbf{Explaining the difference between train and eval returns:} Early in training, the training return is significantly lower than the eval return. This is because during training, the agent uses an $\epsilon$-greedy exploration strategy with a high $\epsilon$ value (close to 1.0), meaning most actions are taken randomly, which leads to poor performance. In contrast, during evaluation, the agent acts greedily (or with a very small $\epsilon$), using the learned Q-values to select the best actions. As training progresses and $\epsilon$ decays, the training return gradually approaches the eval return because the agent explores less and exploits its learned policy more. Additionally, eval episodes are averaged over multiple rollouts, which can lead to higher and more stable returns compared to individual training episodes.
...
We trained SAC on HalfCheetah-v4 with a fixed temperature of $\beta = 0.1$. As shown in Figure~\ref{fig:halfcheetah}, the agent achieves a peak eval return of 9254, well above the 6000 target.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{report_fig5_halfcheetah_sac.png}
\caption{HalfCheetah-v4 SAC with fixed temperature: Eval return over environment steps.}
\label{fig:halfcheetah}
\end{figure}

\subsection{Automatic Temperature Tuning (Section 3.5)}

Figure~\ref{fig:autotune} compares fixed-temperature and auto-tuned SAC on HalfCheetah-v4, and shows the evolution of the learned temperature $\alpha$ over training.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{report_fig6_halfcheetah_autotune.png}
\caption{Left: Eval return comparison between fixed ($\beta=0.1$) and auto-tuned SAC on HalfCheetah-v4. Right: Learned temperature $\alpha$ over training for the auto-tuned run.}
\label{fig:autotune}
\end{figure}

\textbf{Does auto-tuning improve or achieve comparable performance to the fixed temperature?}
Auto-tuning achieves comparable performance to the fixed temperature setting. Both reach peak returns around 9250--9300, with the auto-tuned version showing slightly smoother learning in the early stages. Since the fixed temperature ($\beta = 0.1$) is already well-tuned for HalfCheetah, the main advantage of auto-tuning is eliminating the need for manual temperature selection.

\textbf{How does the temperature evolve during training?}
The temperature initially drops sharply from 0.1 to approximately 0.025 in the first $\sim$100k steps, then gradually increases back to around 0.11 where it stabilizes for the remainder of training.

\textbf{Why might the temperature change in this particular way for HalfCheetah?}
The initial decrease occurs because early in training, the policy is nearly random and already has high entropy, so the algorithm reduces the entropy bonus to allow the critic to learn more accurate Q-values without excessive noise. As training progresses and the policy becomes more deterministic (converging toward exploitation), the temperature increases to maintain sufficient exploration and prevent premature convergence. The eventual stabilization around 0.11 (close to the manually-tuned value of 0.1) indicates that the dual gradient descent has found a balance between exploration and exploitation that satisfies the target entropy constraint, which for HalfCheetah's 6-dimensional action space is $\mathcal{H}_{\text{target}} = -6$.
