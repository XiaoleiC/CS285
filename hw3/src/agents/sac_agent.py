...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = nn.Parameter(torch.tensor(np.log(temperature if temperature > 0 else 1e-8), dtype=torch.float32))
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_action_entropy = -next_action_distribution.log_prob(next_action)  # shape (batch_size,)
                next_qs += self.get_temperature() * next_action_entropy.unsqueeze(0)  #
                # Hint: next_qs = ...
                # ENDTODO
